...
li3i(x) = x/2 * first(quadgk(t -> t^2/(exp(t)-x),0,1000,rtol=1e-3))
li2i(x) = - first(quadgk(t -> log(1-t)/t,0,x,rtol=1e-3))
li1(x) = - log(1-x)
...
Vt(s,eta) = @. N*P^3/(2*pi^3) * real(li3i(-ℯ^(-pi/P*(abs(s)+im+im*eta)))-li3i(-ℯ^(-pi/P*(abs(s)-im+im*eta)))) # factor maybe N*P^2/(pi^2)
...
de2Vt(s,eta) = @. N*P/(2*pi) * real(li1(-ℯ^(-pi/P*(abs(s)-im+im*eta)))-li1(-ℯ^(-pi/P*(abs(s)+im+im*eta))))
dsVt(s,eta) = @. N*P^2/(2*pi^2) * real(li2i(-ℯ^(-pi/P*(abs(s)-im+im*eta)))-li2i(-ℯ^(-pi/P*(abs(s)+im+im*eta))))
